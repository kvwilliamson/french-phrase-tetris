    ...
    try {
        if (dropIndex >= currentWordGroups.length) {
            if (nextWordGroups.length > 0) {
                ...
                const noWordsText = scene.add.text(scene.cameras.main.width / 2, scene.cameras.main.height / 2, 'No More Words', { fontSize: '48px', color: '#ff0000' }).setOrigin(0.5);
        ...
        const errorText = scene.add.text(scene.cameras.main.width / 2, scene.cameras.main.height / 2, 'Error: Game Paused', { fontSize: '48px', color: '#ff0000' }).setOrigin(0.5);
